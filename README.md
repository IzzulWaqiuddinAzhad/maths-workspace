...
Angle explorations retain the library and diagram/sidebar layout. `angle-model.js` owns independent ray IDs, creation order, derived regions, five manual quiz tiers and an arithmetic-only calculator parser. `angle-ui.js` connects those models to the existing explorer interface. Dragging uses the shared `screenToWorld` conversion with the explorer's own viewport; notebook pan/zoom does not move the dialog. Drag empty explorer space to pan, scroll to zoom, and use Reset view to recenter. Straight-line exploration starts with no internal rays; full-circle starts with one movable ray. Add splits the largest region; Remove reverses creation order. Relationship visibility is explicit and always off in a quiz. Calculator state is independent of question data. Exploration state is session-only, with up to eight rays and a tiny coincidence guard; physical stylus/touch devices still need device testing.

Validation: manual browser checks covered independent handles in both modes, live values, pan/zoom dragging, base states, add/remove, show/hide, all ten mode/tier screens, incorrect/correct submission, and calculator open/close and arithmetic. Automated tests validate 1,000 generated questions across all tiers/modes, pointer transforms, ray identity/removal, totals, rounding, and calculator syntax.
