...
export function mountAngleExplorer({trigger,language=()=> 'en'}={}){
  ...
  let mode='straight',interior=[112],circleRays=[20,145,275],revealed=false,question=null,attempts=0,streak=0,dragIndex=null,libraryOpen=true;
  ...
  function bindCanvas(canvas){
    const pointerAngle=e=>{const r=canvas.getBoundingClientRect();return(Math.atan2(r.height/2-(e.clientY-r.top),e.clientX-r.width/2)*180/Math.PI+360)%360};
    canvas.onpointerdown=e=>{const ang=pointerAngle(e),all=rays();let closest=-1,best=16;all.forEach((a,i)=>{if(mode==='straight'&&(i===0||i===all.length-1))return;const d=Math.abs(((ang-a+540)%360)-180);if(d<best){best=d;closest=i}});if(closest>=0){dragIndex=closest;canvas.setPointerCapture(e.pointerId)}};
    canvas.onpointermove=e=>{if(dragIndex===null||question)return;const ang=pointerAngle(e);if(mode==='straight'){const val=Math.max(1,Math.min(179,180-ang));interior[dragIndex-1]=val;interior.sort((a,b)=>b-a);dragIndex=interior.indexOf(val)+1}else{circleRays[dragIndex]=ang;circleRays.sort((a,b)=>a-b);dragIndex=circleRays.indexOf(ang)}renderCanvas(canvas,question?.labels??null);const equationNode=dialog.querySelector('.total-card strong');if(equationNode)equationNode.textContent=equation(regions(),total())};
    canvas.onpointerup=canvas.onpointercancel=()=>dragIndex=null;
  }
  function renderLibrary(){dialog.innerHTML=`<button class="close explorer-close" aria-label="Close">×</button><div class="explorer-library"><span class="explorer-kicker">${tr('Exploration library','Perpustakaan penerokaan')}</span><h2>${tr('Explore mathematics','Terokai matematik')}</h2><details open><summary>${tr('Lines and Angles','Garis dan Sudut')}</summary><div class="library-items"><button data-activity="straight"><strong>${tr('Angles on a Straight Line','Sudut pada Garis Lurus')}</strong><span>${tr('Explore a total of 180°','Teroka jumlah 180°')}</span></button><button data-activity="circle"><strong>${tr('Angles Around a Point','Sudut di Sekeliling Titik')}</strong><span>${tr('Explore a total of 360°','Teroka jumlah 360°')}</span></button></div></details></div>`;dialog.querySelector('.explorer-close').onclick=()=>dialog.close();dialog.querySelectorAll('[data-activity]').forEach(b=>b.onclick=()=>{mode=b.dataset.activity;libraryOpen=false;question=null;revealed=false;render()})}
  function render(){if(libraryOpen){renderLibrary();return}const exp=EXPLORATIONS[mode],r=regions(),quiz=!!question;dialog.innerHTML=`<button class="close explorer-close" aria-label="Close">×</button><div class="explorer-head"><div><button class="library-back" id="libraryBack">← ${tr('Library','Pustaka')}</button><span class="explorer-kicker">${tr('Lines and Angles','Garis dan Sudut')}</span><h2>${quiz?tr('Quiz Yourself','Uji Diri'):tr(exp.title,mode==='straight'?'Sudut pada Garis Lurus':'Sudut di Sekeliling Titik')}</h2></div><div class="explorer-tabs"><button data-mode="straight" class="${mode==='straight'?'active':''}">${tr('Straight line','Garis lurus')}</button><button data-mode="circle" class="${mode==='circle'?'active':''}">${tr('Around a point','Sekeliling titik')}</button></div></div><div class="explorer-grid"><section><canvas class="angle-stage" aria-label="Interactive angle diagram"></canvas><p class="angle-instruction">${quiz?tr(`Find ${question.symbol}.`,`Cari ${question.symbol}.`):tr('Drag a blue ray to explore.','Seret sinar biru untuk meneroka.')}</p></section><section class="explorer-side">${(revealed||quiz)?`<div class="total-card"><span>${tr('Angle relationship','Hubungan sudut')}</span><strong>${quiz?equation(regions(),total()):equation(r,total())}</strong></div>`:''}${quiz?`<div class="quiz-card"><p>${question.algebra?tr('The labels are expressions. Find the value of the symbol.','Label ialah ungkapan. Cari nilai simbol.') :tr('Use the diagram and angle-sum relationship.','Gunakan rajah dan hubungan jumlah sudut.')}</p><label>${question.symbol} = <input id="angleAnswer" type="number" inputmode="decimal" step="any" autofocus></label><button class="primary" id="submitAngle">${tr('Submit','Hantar')}</button><p id="quizFeedback" class="feedback" aria-live="polite"></p><button id="showWorking" hidden>${tr('Show working','Tunjuk cara kerja')}</button><pre id="working" hidden>${question.steps.join('\n')}</pre><button id="nextQuestion">${tr('New question','Soalan baharu')}</button></div>`:`<div class="explorer-actions"><button class="primary" id="addRay">+ ${tr('Add Ray','Tambah sinar')}</button><button id="reveal">${tr('Reveal relationship','Dedahkan hubungan')}</button><button id="startQuiz">${tr('Quiz Yourself','Uji Diri')}</button></div><p class="explorer-note">${tr('Move the rays, then reveal the relationship.','Gerakkan sinar, kemudian dedahkan hubungan.')}</p>`}</section></div>`;const canvas=dialog.querySelector('canvas');renderCanvas(canvas,quiz?question.labels:null);bindCanvas(canvas);dialog.querySelector('.explorer-close').onclick=()=>dialog.close();dialog.querySelector('#libraryBack').onclick=()=>{libraryOpen=true;render()};dialog.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{mode=b.dataset.mode;question=null;revealed=false;render()});if(quiz){dialog.querySelector('#submitAngle').onclick=submit;dialog.querySelector('#angleAnswer').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();submit()}};dialog.querySelector('#nextQuestion').onclick=makeQuiz;dialog.querySelector('#showWorking').onclick=()=>dialog.querySelector('#working').hidden=false}else{dialog.querySelector('#addRay').onclick=addRay;dialog.querySelector('#reveal').onclick=()=>{revealed=true;render()};dialog.querySelector('#startQuiz').onclick=makeQuiz}}
  trigger.onclick=()=>{libraryOpen=true;question=null;revealed=false;dialog.showModal();render()};return {open:()=>trigger.click(),definitions:EXPLORATIONS};
}
