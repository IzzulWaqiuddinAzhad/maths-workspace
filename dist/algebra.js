...
export function drawFunctions(ctx,o,metrics){
 ...
 const {axis,op,at,difference,accept}=relation,colour=item.colour||'#2563eb';ctx.strokeStyle=colour;ctx.fillStyle=colour;ctx.lineWidth=2;ctx.lineCap='round';ctx.setLineDash(op==='<'||op==='>'?[.01,5]:[]);
 if(axis){const vertical=axis==='x',from=vertical?o.yMin:o.xMin,to=vertical?o.yMax:o.xMax,steps=Math.min(1600,Math.max(100,Math.ceil(vertical?h:w))),point=(t,v)=>vertical?{x:origin.x+v*unit,y:origin.y-t*unit}:{x:origin.x+t*unit,y:origin.y-v*unit};let previous=null,connected=false;ctx.beginPath();for(let i=0;i<=steps;i++){const t=from+(to-from)*i/steps,v=vertical?at(0,t):at(t,0),p=point(t,v);if(!Number.isFinite(v)){previous=null;connected=false;continue;}if(op!=='='){ctx.save();ctx.globalAlpha*=.13;const low=op.startsWith('<');if(vertical){const x=Math.max(left,Math.min(left+w,p.x));ctx.fillRect(low?left:x,p.y-(h/steps),low?x-left:left+w-x,h/steps+1);}else{const y=Math.max(top,Math.min(top+h,p.y));ctx.fillRect(p.x,low?y:top,w/steps+1,low?top+h-y:y-top);}ctx.restore();}
 const mid=previous?(vertical?at(0,(previous.t+t)/2):at((previous.t+t)/2,0)):NaN,range=vertical?o.xMax-o.xMin:o.yMax-o.yMin;
 if(previous&&Number.isFinite(mid)&&Math.abs(mid-(previous.v+v)/2)<range*.2&&Math.abs(v-previous.v)<range*2){if(!connected)ctx.moveTo(previous.p.x,previous.p.y);ctx.lineTo(p.x,p.y);connected=true;}else connected=false;previous={p,t,v};}ctx.stroke();
