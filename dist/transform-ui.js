...
import {
  openTransformations, graphToScreen, screenToGraph, GRID_UNIT, snapPoint, translatePoint,
  ...
  leverAngle, snapRotation, ReflectionScrub, newAnnotation, eraseAnnotations,
} from './transform-model.js?v=25';
import { createTransformationRenderer } from './transform-render.js?v=25';
...
export function mountTransformations(host, { language = () => 'en', back }) {
  ...
  host.insertAdjacentHTML('beforeend', `
    ...
    <div class="tf-layout">
      <section class="tf-board" aria-label="${tr('Transformation graph', 'Graf transformasi')}">
        ...
        <div id="tfNudges" class="tf-object-arrows" hidden>${button('tfNudgeUp', '↑', '↑', `aria-label="${tr('Translate up one unit', 'Translasi satu unit ke atas')}"`)}${button('tfNudgeLeft', '←', '←', `aria-label="${tr('Translate left one unit', 'Translasi satu unit ke kiri')}"`)}${button('tfNudgeRight', '→', '→', `aria-label="${tr('Translate right one unit', 'Translasi satu unit ke kanan')}"`)}${button('tfNudgeDown', '↓', '↓', `aria-label="${tr('Translate down one unit', 'Translasi satu unit ke bawah')}"`)}</div>
      ...
      <aside class="tf-side">
        <div class="tf-object-select"><label for="tfObjects">${tr('Selected object', 'Objek dipilih')}</label><select id="tfObjects"></select></div>
  ...
  function draw() {
    ...
    renderGraph({ view, objects: data().objects.map(o => drag?.id === o.id && drag.points ? { ...o, points: drag.points } : o), selected, preview, annotations: workingAnnotations ?? data().annotations, transform, centre, centreLabel: tr('Centre', 'Pusat'), mirror, guides: transform === 'reflection' ? guides && flip === 1 : guides, coordinates, labels: showLabels, vertex, angle, draft: polygon, ink, drawLine });
    get('tfEmpty').hidden = data().objects.length > 0 || tool !== 'select';
    const o = selectedObject(), nudge = get('tfNudges'); nudge.hidden = transform !== 'translation' || !o || tool !== 'select';
    if (!nudge.hidden) {
      const points = (preview || o).points.map(p => graphToScreen(p, view)), xs = points.map(p => p.x), ys = points.map(p => p.y);
      const cx = (Math.min(...xs) + Math.max(...xs)) / 2, cy = (Math.min(...ys) + Math.max(...ys)) / 2;
      const positions = { tfNudgeUp: [cx, Math.min(...ys) - 35], tfNudgeDown: [cx, Math.max(...ys) + 35], tfNudgeLeft: [Math.min(...xs) - 35, cy], tfNudgeRight: [Math.max(...xs) + 35, cy] };
      for (const [id, [x, y]] of Object.entries(positions)) { const b = get(id); b.style.left = `${Math.max(24, Math.min(board.clientWidth - 24, x))}px`; b.style.top = `${Math.max(24, Math.min(board.clientHeight - 70, y))}px`; }
    }
  ...
  function sync() {
    ...
    const words = angle === 0 ? '0°' : `${formatNumber(Math.abs(angle))}° ${angle > 0 ? tr('anticlockwise', 'lawan arah jam') : tr('clockwise', 'ikut arah jam')}`;
    ...
    get('tfKeep').disabled = !imagePoints() || !!animation || (transform === 'reflection' && !reflected);
  ...
  function activate(type) {
    ...
    say(type === 'rotation' ? 'Choose the rotation centre on the graph or enter its coordinates.' : type === 'reflection' ? 'Draw a mirror line or enter its equation.' : 'Use the blue arrows or edit the column vector.', type === 'rotation' ? 'Pilih pusat putaran pada graf atau masukkan koordinatnya.' : type === 'reflection' ? 'Lukis garis pantulan atau masukkan persamaannya.' : 'Gunakan anak panah biru atau sunting vektor lajur.'); sync();
  }
  ...
  function keepImage() {
    const source = selectedObject(), points = imagePoints(); if (!source || !points || animation) return;
    if (data().objects.length >= 100) { say('Maximum 100 objects.', 'Maksimum 100 objek.'); return; }
    const id = crypto.randomUUID(), labels = imageLabels(source.labels, data().objects);
    store.transact(d => d.transformation.objects.push({ id, name: source.name.slice(0, 39) + '′', points, labels, image: true }));
    choose(id); say('Image kept. Choose another transformation to continue.', 'Imej dikekalkan. Pilih transformasi lain untuk meneruskan.');
  }
  ...
  get('tfVectorReset').onclick = () => { vector = { x: 0, y: 0 }; sync(); };
  ...
  get('tfDelete').onclick = () => { if (!selected) return; store.transact(d => { d.transformation.objects = d.transformation.objects.filter(o => o.id !== selected); }); choose(null); };
  ...
  const lever = get('tfLever'); let leverPointer = null;
  const moveLever = e => { if (e.pointerId !== leverPointer) return; const r = lever.getBoundingClientRect(); setAngle(leverAngle(e.clientY, r.top, r.height)); };
  lever.onpointerdown = e => { if (!centre || e.button !== 0) return; e.preventDefault(); lever.focus({ preventScroll: true }); leverPointer = e.pointerId; lever.setPointerCapture(e.pointerId); moveLever(e); };
  ...
  function onDown(e) {
    ...
    if (transform === 'reflection' && mirror) {
      ...
      if (distance(p, lineFoot(p, mirror.line)) < tolerance * .7) { drag = { type: 'mirrorMove', pointer: e.pointerId, start: q, handles: structuredClone(mirror.handles) }; stopAnimation(); reflected = false; flip = 0; sync(); return; }
    }
    const preview = previewObject();
    if (transform === 'translation' && preview && objectHit(p, preview, tolerance)) { drag = { type: 'image', pointer: e.pointerId, start: p, vector: { ...vector } }; return; }
    const o = selectedObject();
  ...
  function onMove(e) {
    ...
    else if (drag.type === 'mirrorMove') { const delta = { x: q.x - drag.start.x, y: q.y - drag.start.y }; setMirror(drag.handles.map(h => translatePoint(h, delta))); sync(); }
    else if (drag.type === 'image') { const delta = snapPoint({ x: p.x - drag.start.x, y: p.y - drag.start.y }, snap); vector = translatePoint(drag.vector, delta); sync(); }
  ...
  function onEnd(e, cancelled = false) {
    ...
    drag = null;
    if (action.type === 'pen' && ink) { const stroke = ink; ink = null; store.transact(d => d.transformation.annotations.push(stroke)); }
