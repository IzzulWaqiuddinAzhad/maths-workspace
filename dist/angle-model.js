...
function split(total,count,min=15,rng=Math.random){const values=Array(count).fill(min);for(let i=0;i<total-count*min;i++)values[Math.floor(rng()*count)]++;return values;}
export function quizQuestion(mode,tier,rng=Math.random){
 ...
 if(!algebra){const count=tier===1?(mode==='straight'?2:3):rand(3,5),values=split(total,count,15,rng);const unknown=rand(0,count-1);answer=values[unknown];terms=values.map((v,i)=>({a:i===unknown?1:0,b:i===unknown?0:v}));}
